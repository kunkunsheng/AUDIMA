...
from model.nets.blocks import t2i_modulate, CaptionEmbedder, AttentionKVCompress, MultiHeadCrossAttention, T2IFinalLayer, TimestepEmbedder, SizeEmbedder
from model.nets.dit import DiT, get_2d_sincos_pos_embed
...
class Dimba(DiT):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DimbaBlock(
                ...
                input_size=(256 // patch_size, 16 // patch_size),
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, **kwargs):
        ...
        ).unsqueeze(0).to(x.cuda()).to(self.dtype)

        x = self.x_embedder(x) + pos_embed  # (N, T, D), where T = H * W / patch_size ** 2  4，1024，512
...
def Dimba_L_2(**kwargs):
    return Dimba(depth=28, hidden_size=512, patch_size=2, num_heads=16, **kwargs)
...
if __name__ == "__main__":
    ...
    x = torch.rand(2, 8, 256, 16).cuda()
    t = torch.rand(2, 512).cuda()
    y = torch.rand(2,1,512).cuda()
    dimaba = Dimba().cuda()
    OUT = dimaba(x,t,y)
    print(OUT)