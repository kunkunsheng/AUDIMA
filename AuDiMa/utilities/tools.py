...
def synth_one_sample_val(
    ...
    mel_target = targets[6][index, :mel_len].detach().transpose(0, 1).contiguous()

    mel_prediction = predictions[0][index, :mel_len].detach().transpose(0, 1).contiguous()
    postnet_mel_prediction = predictions[1][index, :mel_len].detach().transpose(0, 1).contiguous()
...
def synth_samples(targets, predictions, vocoder, model_config, preprocess_config, path):
    ...
    for i in range(len(predictions[1])):
        ...
        mel_prediction = predictions[1][i, :mel_len].detach().transpose(0, 1).contiguous()
    ...
    mel_predictions = predictions[1].transpose(1, 2).contiguous()
