...
class UnFlexiPatchEmbed(nn.Module):
    def __init__(
        self,
    ):
        super().__init__()

        self.conv1 = nn.Conv2d(in_channels=512, out_channels=64, kernel_size=(8, 1), stride=(1, 1))
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(1, 1), stride=(1, 1))
        self.conv3 = nn.Conv2d(in_channels=64, out_channels=1024, kernel_size=(1, 1), stride=(1, 1))

    def forward(self, x):
        B = x.size(0)
        x = x.transpose(1, 2)  
        x = x.view(B, 512, 8, 64)  
        output_tensor = self.conv1(x)
